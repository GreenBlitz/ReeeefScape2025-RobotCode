...
public class VisionFilters {
	...
	public static Filter<VisionData> isYawAtAngle(Supplier<Optional<Rotation2d>> wantedYawSupplier, Rotation2d yawTolerance) {
		return (visionData) -> {
			if(wantedYawSupplier.get().isEmpty()) {
				return false;
			}
			return ToleranceMath
					.isNearWrapped(wantedYawSupplier.get().get(), Rotation2d.fromRadians(visionData.getEstimatedPose().getRotation().getZ()), yawTolerance);
		};
	}
