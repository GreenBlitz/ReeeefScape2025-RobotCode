...
public class ScoringHelpers {
	...
	public static Command fullyScoreToChosenLevel(Robot robot) {
		return new SelectCommand<>(
			Map.of(
				ScoreLevel.L1,
				robot.getRobotCommander().fullyScore(ScoreLevel.L1),
				ScoreLevel.L2,
				robot.getRobotCommander().fullyScore(ScoreLevel.L2),
				ScoreLevel.L3,
				robot.getRobotCommander().fullyScore(ScoreLevel.L3),
				ScoreLevel.L4,
				robot.getRobotCommander().fullyScore(ScoreLevel.L4)
			),
			() -> targetScoreLevel
		);
	}
	...
	public static void log(String logPath) {
		...
		Logger.recordOutput(logPath + "/TragetPose",new Pose2d(
				Field.getCoralPlacement(getTargetBranch()), new Rotation2d()));
	}
