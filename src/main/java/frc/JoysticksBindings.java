...
public class JoysticksBindings {
	...
	private static void secondJoystickButtons(Robot robot) {
		...
		usedJoystick.getAxisAsButton(Axis.RIGHT_TRIGGER).onTrue(robot.getRobotCommander().setState(RobotState.PRE_SCORE));
		...
		usedJoystick.START.onTrue(robot.getRobotCommander().setState(RobotState.DRIVE));
	}
...
//		robot.getSwerve().applyCalibrationBindings(usedJoystick, () -> robot.getPoseEstimator().getEstimatedPose());
		usedJoystick.R1.onTrue(robot.getRobotCommander().scoreWithoutReleaseThanScoreSequenceTest());
